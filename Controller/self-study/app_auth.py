...
#allowed IP
ip_list = ["1.1.1.1","1.1.1.2","1.1.1.3","1.1.1.4","192.168.2.4","192.168.2.3","1.1.2.2","1.1.2.3","1.1.2.4","4.1.1.1","1.2.1.2","1.2.1.3","1.2.1.4","1.2.2.1","1.2.2.2","1.2.2.3","1.2.2.4","192.168.2.4"]
...
class LearningSwitch (object):
  ...
  def _handle_PacketIn (self, event):
    ...
    #packet type ipv4 and ICMP  
      ...
      #Authentication and Modification of flow entries   
      if ip_packet.protocol == ip_packet.TCP_PROTOCOL:
        ...
        if (dst_mac == web_mac and dst_ip == web_ip and dst_port == 80):
          ...
          print(x,m)
          print ("++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++")
          print "Request reached controller with destination mac as web-server and authentication starts"
          print ("dpid of the ovs:",event.dpid)
          print ("source IP & Destination IP",src_ip,dst_ip)
          if src_ip in ip_list:
            print ("Authenticaton is done and it is a valid client")
            print ("dpid of the ovs:",event.dpid)
            print ("source IP & Destination IP",src_ip,dst_ip)
        ...
        elif (dst_ip == IPAddr("192.168.2.255") and dst_mac == EthAddr("ff:ff:ff:ff:ff:ff") and dst_port==80):
          print ("Authenticaton is done and it is a valid client")
          print ("dpid of the ovs:",event.dpid)
          print ("source IP & Destination IP",src_ip,dst_ip)
          print ("source MAC & Destination MAC",src_mac,dst_mac)
